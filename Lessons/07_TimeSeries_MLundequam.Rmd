...
Use the models to generate future scenarios and make decisions based on them.
...
Stochastic trend - detected by ADF test
...
These tests allow you to verify whether or not there is an overall trend increase
...
* **Seasonal Mann-Kendall**: seasonality, non-parametric `SeasonalMannKendall` (package: Kendall)
  looks at each season of the year and determines whether each season has a trend
* **Spearman Rho**: no seasonality, non-parametric, missing data allowed. Function: `cor.test(method="spearman")` (package: stats)
  This one can only handle non-season data - good for double checking - affirming assumptions
  if you do have seasonal data - you can use this to look at monthly means and see how they change year to year
...
wind_data_ts <- ts(wind_data_clean$AWND.clean,
                   ...
                   frequency=12) #tells when to start repeating the data, if you were using daily data, the                                       frequency would be 365

### we will shift back and forth between time series objects and data frames because different commands will only accept one or the other
