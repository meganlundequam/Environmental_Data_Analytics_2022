...
O3.onesample
# we are checking to see if our sample follows a normal distribution - if they do, they will follow this line. With the ends straying, we can tell that the tail is skewing. But this is not the only test - the shapiro test can also test - but sometimes the shapiro test can fail.
...
```{r}
#Exercise 2:
# if you use color instead of fill, only the line will be colored
O3.density <-
  ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) +
  geom_density(alpha = 0.5) +
  scale_fill_manual(values = c("darkgreen", "darkblue")) +
  labs(x = "Ozone AQI Values", y = "Density", fill = "")
print(O3.density)

O3.boxplot <-
  ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) +
    geom_boxplot() +
    stat_summary(geom = "text", fun.y = max, vjust = -1, size = 4, label = c("b", "a")) + #this is specifying where you want the labels to show up
    labs(x = "", y = "Ozone AQI Values") +
    ylim(0,150)
print(O3.boxplot)
...
# violin gives information about probability density function and dispersion
...
Low R-squared means regression is not very accurate - something is making y change that is not explained by the variables selected.
If the variables are not explaining the model, remove them.
...
PM2.5.Ozone.regression <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(PM2.5.Ozone.regression)

# formula: y = 0.38x + 15.6
# pvalue is < 0.5 so this is a good relation
# r-squared is only 13% - means that these variables explain 13% of the variability of PM2.5 - they have a relationship, but only 13%
# this tells us we need more variables in our regression to get a better explanation.

#Exercise 4: Build a scatterplot. Add a line and standard error for the linear regression. 

PM2.5byOzone <- 
  ggplot(PM2.5.Ozone.regression, aes(x = Ozone, y = PM2.5)) +
  ylim(0, 100) +
  geom_point() +
...
library(corrplot) # gives you correlation among any variable you have in dataframe
PM2.5.other <- 
  EPAair %>%
  select(meanLat:Ozone) %>%
  na.omit()
EPAairCorr <- cor(PM2.5.other)
...
OzonebyALL.regression <- lm(data = EPAair.subsample, PM2.5 ~ Ozone + Year + Month + meanLng + meanLat)
step(OzonebyALL.regression) # lets you see which variables are good candidates
# even though they might not show great correlation from the plot, they are adding something to the model
...
PM2.5.other.regression <- lm(data = EPAair, PM2.5 ~ Month + meanLat + meanLng + Ozone)
summary(PM2.5.other.regression)
