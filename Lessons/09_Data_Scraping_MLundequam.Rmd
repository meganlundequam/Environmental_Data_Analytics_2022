...
NCDEP has place you can see water withdrawal transfer registrations.
 ...
 * View the data we can scrape on this page. 
 industrial water user - report annual daily water withrawal average.
 with this data - you can look to see how much water is withdrawn from different industries.
 
 TO SCRAPE - we are basically pointing our code to a specific part of the web page
...
EXERCISE: Open the source of the web page in your browser (<ctrl>-<u>). (or right click --> "view page source")
* Do you see the <head> and <body> sections?
  Values are between tags - start with <___> and end with </___>
  Java scripts tell what to do and link to other pages that tell what to do
...
df_withdrawals <- data.frame("Month" = rep(1:12), #rep command creates a sequence of numbers 1-12
                             "Year" = rep(2000,12), #rep command makes vector of 12 repeats of the year 2000
                             "Avg_Withdrawals_mgd" = as.numeric(avg_withdrawals), #add and convert to numeric
                             "Max_Withdrawals_mgd" = as.numeric(max_withdrawals)) #add and convert to numeric
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant, #!! means this refers to a variable
         Facility_name = !!the_facility_name,
         Facility_type = !!the_facility_type,
         Date = my(paste(Month,"-",Year))) #converting month and year to date
...
print(the_scrape_url) #paste0 eliminates adding a space

#Retrieve the website contents 
the_website <- read_html(the_scrape_url)

#Set the element address variables (determined in the previous step with selector gadget)
...
scrape.it <- function(the_year, the_facility){ #instead of hard coding these elements, they are now 'inputs'
  ...
  #Sys.sleep(1) #uncomment this if you are doing bulk scraping! BASICALLY tells R to wait for a second - tells R to run more slowly - bad etiquite to ask for data too quickly - could signal an attack/hack
...
R can iterate using either "lapply" or Purrrs' "map" functions. (Purr is tidyverse version)
```{r scrape.multiple.years.of.data}
#Set the inputs to scrape years 2015 to 2020 for the site "0004-0001"
the_years = rep(2018:2020) #creating a vector of years
my_facility = '0004-0001' #set facility ID

#Use lapply to apply the scrape function
the_dfs <- lapply(X = the_years,
                  FUN = scrape.it, #run this function for each year in the vector, specifying the facility below
                  the_facility=my_facility) #could also just set this as a value

#OR, use purrr's map function
the_dfs <- map(the_years,scrape.it,the_facility=my_facility) #same thing - iterated a different way
...
the_facility_ids <- the_main_website %>% 
  html_nodes('#content a') %>%    #Extract a list of html nodes taged with "content a" (the 'view report' link)
  html_attr("href") %>%           #Pull the "href" attribute from the node (as opposed to the text) (web address associated with the links)
...
When you run nodes - you get this:
<a href="https://www.ncwater.org/WUDC/app/WWATR/report/view/0218-0030/2021">View Report</a>
where we first have the tag, value, closing tag, web address is href
...
the_facility_ids_subset <-  sample(the_facility_ids,20) #grabbing 20 random IDs
...
dfs_2020 <- map2(the_years, the_facility_ids_subset, scrape.it) #this function iterates through these lists in parallel - first two inputs are the lists - iterating through these 20 facilities
...
the_years = c(2018,2019,2020) #map 2 would pull the facility data for 2018, then the second for 2019, etc. if we want the 3 years for each facility, we need to use the cross2 command
...
the_df <- cross2(the_years,the_facilities) %>% 
  map(lift(scrape.it)) %>% #map + lift mapping combos
  bind_rows()
