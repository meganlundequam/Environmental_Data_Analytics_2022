...
#b Note that on both data sets that is a column with the month name and average. Convert it to a Month column by separating the string, i.e., month in one column and the word "Average" on another. Hint: use the separate() function in the pipe.
  #**** lubridate cannot fix this - "space" is a column delimiter, use separate, name the names of each column that will contain the character name
...
Rainfall_BR_processed <-
  ...
  mutate( Date = my(paste0(Month,"-",Year))) %>% #this is a lubridate function - parsing into a date option
...
# allows you to actually see if there are any missing data points
...
#*** specifying the start argument is what allows the years to show up
f_month <- month(first(BRA_complete$Date))
f_year <- year(first(BRA_complete$Date))
temperature_ts <- ts(BRA_complete$Temperature_C,
                   start=c(f_year,f_month),
                   frequency=12) 
rainfall_ts <- ts(BRA_complete$Rainfall,
                   start=c(f_year,f_month),
                   frequency=12) 
...
#decompose
temperature_decomp <- stl(temperature_ts,s.window = "periodic")
plot(temperature_decomp)

rainfall_decomp <- stl(rainfall_ts,s.window = "periodic")
plot(rainfall_decomp)

# Run SMK test
temperature_trend1 <- Kendall::SeasonalMannKendall(temperature_ts)

# Inspect results
temperature_trend1
summary(temperature_trend1)

temperature_trend2 <- trend::smk.test(temperature_ts)
# Inspect results
temperature_trend2
summary(temperature_trend2)
...
# this is just for visualization - allows you to access individual series so you can remove them if you wanted to perform a certain test or 

# We can extract the components and turn them into data frames
temperature_Components <- as.data.frame(temperature_decomp$time.series[,1:3])

temperature_Components <- mutate(temperature_Components,# this command adds these columns back in
        Temperature = BRA_complete$Temperature_C,     
...
rainfall_Components <- mutate(rainfall_Components,
        Rainfall = BRA_complete$Rainfall,     
        Date = BRA_complete$Date)
...
ggplot(temperature_Components) +
  geom_line(aes(y = Temperature, x = Date),  size = 0.25) +
