...
Second line - layers for plotting.
Layers are basically the type of plot.
...
# when you specify color in the aes() section, you are adding a layer to the plot rather than changing the color of the points.
# instead, you need to run it outside of the aes() command but inside the geom_point() command
# Fix this code
...
# specifying color with Site.Name - inside aes() becuase it is adding a layer this time
# ggplot automatically adds a legend
PMplot <- 
...
# facets separates the plots according to the variable selected
PMplot.faceted <-
...
# subset allows you to grab certain data within the variables
# facet grid makes graphs appear like a matrix
PMplot.faceted2 <- 
...
# basically connecting the points
PMplot.line <- 
...
lightvsDO <- 
  ...
  geom_point()
print(lightvsDO)
# not a good visualization - need to adjust the axis.
...
# scale_y_reverse reverses the order of the y axis values
tempvsdepth <- 
...
NvsP <-
  ...
  geom_abline(aes(slope = 16, intercept = 0)) # adding a line that has a specfic slope and intercept
print(NvsP)
...
# Dot plot
# stackdir - stacking over the center
# position - dodge calls for no over laying
Nutrientplot4 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
# Violin plot
# box only shows summary stats - but violin also shows density - can see the difference of the density for each nutrient 
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
# adding polygon to each nutrient
# problems: no legend, xaxis only has the label for the first variable
# not a good way to do this and hard - have to specify each one
Nutrientplot6 <-
...
# gathered - one column with concentrations and one column with nutrient name
# much better approach for frequency plots with more than one variable
Nutrientplot7 <-   
...
# spaghetti plots - when its just a bunch of lines crossing over eachother
# See <https://www.data-to-viz.com/caveat/spaghetti.html> for more info.

# Ridgeline plot
# one ridge for each lake - fill to color inside of curve
Nutrientplot6 <-
