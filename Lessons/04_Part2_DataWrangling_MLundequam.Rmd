...
# Format sampledate as date - still have to process it again
...
# use function droplevels to make them go away
...
# sample date is a factor because we did asfactor function - have to change again
...
NTL.nutrient.data.PeterPaul <- 
  ...
  select(lakeid:daynum, month, sampledate:comments) %>% # reordering again
  drop_na(depth) # dropping all na's
...
# Remove columns that are not of interest for analysis - just select certain range
NTL.phys.data.PeterPaul.subset <- select(NTL.phys.data.PeterPaul, 
                                         lakename:irradianceDeck)

# if you dont want to do a range, you can just write out what you want  
...
```{r}
# Gather nutrient data into one column using old command
...
# assign new variable, select columns representing nutrients, spread into names and the values for that nutrient
# called pivot longer bc now it has more rows
...
# there are a lot of na's in concentration value column - those rows are not adding any information into the dataset so we can remove those rows
...
# moved name back to column and value back into the rows
...
# this combined the common columns, and then added the new columns from each
...
Rbind is short for row bind. Binds elements by row.
...
```{r}
# copies to an object the files we want to read
...
dim(Trap)
dim(LitterTrap)
# we can see that columns were combined, minus the columns they share (19 + 23 - 3 = 39)
...
This is how to creating a table with summary statistics for specific variables across the dataset.

```{r}
NTL.PeterPaul.summaries <- 
  NTL.phys.nutrient.data.PeterPaul %>%
  filter(depth == 0) %>% # to get info on surface
  group_by(lakename, month) %>% 
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% # filter na for only temperature, tn_ug and tp_ug - adding row
