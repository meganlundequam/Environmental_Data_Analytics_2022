...
The `sf` package enables a new data type to be included in our data frames - the **geometry** data type. "Geometry" is how we talk about different types of objects. Geometries are comprised of coordinate pairs that form to create **geometric objects**. These can be:
...
A **coordinate reference system** or **crs** includes the information used to reference the coordinates used in defining a geometry with actual geographic locations. These will be discussed more later. ** thing that allows us to pin this on the earth.
...
Dataframe has a new column of geomoetries and the geom is comprised of coordinates and the coordinates are tagged with w a reference system that allows us to tag it on earth.
...
#Explore
class(GasPlants) #sf object - means we can do spatial analysis with it
names(GasPlants)
str(GasPlants$geometry)

#View using sf plot
plot(GasPlants['STATUS'],pch=16) #plot within sf package
...
sfg_GraingerHall <- st_point(c(-78.9427,36.00465)) #this command creates this point as a sf point
class(sfg_GraingerHall)
mapview(sfg_GraingerHall)

#Upgrade to a simple feature geometry column by adding a crs 
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) #crs ties the coordinate into a place on earth
...
the_buffer <- sf_GraingerHall %>% st_buffer(1) # 1 refers to 1 degree lat long
