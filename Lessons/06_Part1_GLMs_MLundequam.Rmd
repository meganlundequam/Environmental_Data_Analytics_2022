...
par(mfrow = c(2,2), mar=c(4,4,4,4)) #dividing plot screen into 4 cells, 2X2 grid
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000) #get rid of anything greater than 5000 and remove zeroes before logging
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth)
summary(irradiance.regression2)
#reduced residual standard error and r squared
...
``` {r, fig.height = 3, fig.width = 4}
TPregression <- lm(data = subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), #subsetting data set for only paul lake
...
corrplot(PeterPaulCorr, method = "ellipse") #shape and color tells how strong correlation is
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #printing only upper because it is symmetrical
```
...
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4) #consider all variables

#Choose a model by AIC in a Stepwise Algorithm
#We want Stepwise so R moves through all variables
step(TPAIC)
