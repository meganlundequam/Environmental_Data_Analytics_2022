...
summary(Litter.Totals$plotID) #shows this is not a balanced experiment
...
#filtering to run each site location on dry mass
...
# only 2 sites follow normal distribution - 

#results: reject null in all except two: NIWO_057 and NIWO_046
#Run Q-Q Plot - see it is not normal
...
# Format ANOVA as lm
# little bit more detailed - summary from aov object is traditional output - summary from lm will have one row for each categorical variable
# the estimates are with respect to the intercept term
...
# Post-hoc test
# to find which differences between pairs of means are significant
# TukeyHSD() computes Tukey Honest Significant Differences
# calls the result of the anova test
TukeyHSD(Litter.Totals.anova)
#gives each difference of the means and then the p value for the multiple comparisons

# Extract groupings for pairwise relationships
#group=true means group similar means
...
> order them in the same order they are in in the output
...
#comparing means for each of the 24 levels of mean interactions
#onoe-way ANOVA is used because its interaction of two levels
